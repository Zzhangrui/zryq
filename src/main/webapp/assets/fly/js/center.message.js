...
function getReceiveMessage(pageNum) {
    $.ajax({
        url:contextPath+"/fly/user/getReceiveMessage",
        type:'post',
        dataType:'json',
        success:function (res) {
           if(res.list.length>0){
               var data = res.list;
               var len = data.length;
               initReceivePageNum = pageNum;  //当前页码存到全局变量中
               //自定义样式
               laypage.render({
...
function getSendMessage(pageNum) {
    $.ajax({
        url:contextPath+"/fly/user/getSendMessage",
        type:'post',
        dataType:'json',
        success:function (res) {
            if(res.list.length>0){
                var data = res.list;
                var len = data.length;
                initSendPageNum = pageNum;  //当前页码存到全局变量中
                //自定义样式
                laypage.render({
                    elem: 'receive-message-page'
                    , count: res.total
                    , theme: '#1E9FFF'
                    , curr: pageNum
                    , jump: function (obj, first) {
